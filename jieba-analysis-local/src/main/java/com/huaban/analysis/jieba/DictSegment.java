...
//        Character keyChar = new Character(charArray[begin]);
    ...
    public synchronized void unload(char[] charArray) {
        this.unload(charArray, 0, charArray.length);
    }

    public synchronized void unload(char[] charArray, int begin, int length) {
        Character beginChar = new Character(charArray[begin]);
        Character keyChar = charMap.get(beginChar);

        // 搜索当前节点的存储，查询对应keyChar的keyChar，如果没有则创建
        DictSegment ds = lookforSegment(keyChar, 0);
        if (ds != null) {
            // 处理keyChar对应的segment
            if (length > 1) {
                // 词元还没有完全加入词典树
                ds.unload(charArray, begin + 1, length - 1);
            } else if (length == 1) {
